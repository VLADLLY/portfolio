...
<head>
  ...
  <title>Manage Social Data</title>
  <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
<?php include '../includes/admin.php'; ?>
<div class="container">
  <h2>Manage Social Data</h2>
    <a href="add_social.php">Add New</a>
    ...
    <tbody>
      <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_id"])) {
            ...
            $sql_delete = "DELETE FROM social WHERE id = '$delete_id'";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                echo "<td>" . $row["id"] . "</td>";
                echo "<td>" . $row["url"] . "</td>";
                 echo "<td>" . $row["owner_rights"] . "</td>";
                echo '<td><button class="btn btn-danger" onclick="deleteData(' . $row["id"] . ')">Delete</button></td>';
                echo "<td><a href='update_social.php?id=".$row["id"]."' class='btn btn-primary'>Update</a></td>";
                echo "</tr>";
            }
        } else {
